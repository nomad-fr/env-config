...
(defvar pf-font-lock-keywords
  (list
   ;; variables
   '("$[a-zA-Z][a-zA-Z_0-9]*" 0 font-lock-preprocessor-face t)
   
   ;; tables
   '("<[a-zA-Z][a-zA-Z_0-9]*>" 0 font-lock-type-face t)

   ;; macros
   '("\\b\\([a-zA-Z][a-zA-Z_0-9]*\\)\\s *=" 1 font-lock-variable-name-face t)

   ;; reserved operator words
   (list
    (concat "\\(^\\|\\S_\\b\\)\\("
     (mapconcat 'identity
      '("all" "allow-opts" "altq" "anchor" "antispoof" "bandwidth"
        "binat-anchor" "bitmask" "block-policy" "borrow" "cbq"
        "code" "const" "debug" "default" "drop" "dup-to" "ecn"
	"fastroute"
        "fingerprints" "flags" "for" "group" "hfsc" "icmp-type"
        "icmp6-type" "keep" "label" "limit" "linkshare" "load" "log"
        "log-all" "loginterface" "max" "max-mss" "max-src-nodes"
        "max-src-states" "min-ttl" "modulate" "nat-anchor"
        "no-df" "no-sync" "on" "optimization" "os" "port"
        "priority" "priq" "proto" "qlimit" "queue" "quick" "random"
         "rdr-anchor" "realtime" "red" "reply-to" "require-order"
    ...
    (concat "\\(^\\|\\S_\\b\\)\\("
     (mapconcat 'identity
      '("afpovertcp" "afs3-bos" "afs3-callback" "afs3-errors"
        ...
        "exec" "finger" "ftp" "ftp-data" "gopher" "hostnames" "http" "https" "hunt"
        ...
        "sip" "smtp" "smtps" "smux" "snmp" "snmp-trap" "socks" "spamd" "spamd-cfg"
...
(define-derived-mode pf-mode fundamental-mode
  ...
  ;;(setq syntax-table '(pf-mode-syntax-table))
